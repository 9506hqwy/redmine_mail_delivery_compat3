...
module RedmineMailDeliveryCompat3
  module MentionablePatch
    def mail_delivery_compat3_parse_mentions_for_issue_add
      self.mentionable_attributes.each do |attr|
        mail_delivery_compat3_get_mentioned_users(self, nil, send(attr))
      end
    end

    def mail_delivery_compat3_parse_mentions_for_issue_edit
      ...
      details.each do |detail|
        ...
        mail_delivery_compat3_get_mentioned_users(self.journalized, old_content, new_content)
      end
    ...
    def mail_delivery_compat3_parse_mentions_for_wiki_content
      self.mentionable_attributes.each do |attr|
        if self.version == 1
          mail_delivery_compat3_get_mentioned_users(self, nil, send(attr))
        else
          ...
          mail_delivery_compat3_get_mentioned_users(self, previous.text, send(attr))
        end
      end
    end
    ...
    def mail_delivery_compat3_get_mentioned_users(target, old_content, new_content)
...
if Redmine::VERSION::MAJOR >= 5
  Redmine::Acts::Mentionable.include RedmineMailDeliveryCompat3::MentionablePatch
end
